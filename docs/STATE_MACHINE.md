...
- **Connection migratability**: `ConnectionState::is_migratable` returns false when the session carries backend-local state the gateway cannot reconstruct on a migrated backend, so the connection is severed (08006) on failover rather than silently losing it. Triggers set `not_migratable`: `LISTEN "*"` (unreplayable subscription), session `SET`/`RESET`, and — via `SessionChange::NonMigratable` — a temp table (`CREATE TEMP`, `CREATE TEMP TABLE ... AS`), SQL `PREPARE`, a `WITH HOLD` cursor, and (cheap-token prefilter, no parse) `set_config(..., false)` / session advisory locks. Anything the analyzer positively classifies as safe (permanent DDL, plain DML, `SET LOCAL`, transaction-scoped cursors/locks) stays migratable.
