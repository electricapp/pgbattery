...
This is a distributed database system. If you observe inconsistent state (split-brain, lost writes, stuck replication, missing slots), **STOP** and investigate to root cause, then fix it — in the session that found it, however large the fix turns out to be. There is no bug list: a defect that is understood and not fixed is a defect the next run will hit. Write it up only to explain a fix you are landing, never instead of landing one.
...
| Join and rejoin edges  | `testing/join_edges.py`                      | A clone source deposed mid-copy, a clone whose replication slot is taken away (retried by the join, not by the container runtime), orphan slots (bounded against the reconciler's interval), a learner that dies mid-join, and a wiped bootstrap node returning on the cluster's data lineage rather than one it minted. |
...
| ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| `ha-ci.yml`                                            | harness lint + self-tests, `ha-sequential`, `ha-parallel` (matrixed), `ha-controlplane-pr`, `ha-assert-sanity`, `join-edges` | `ha-controlplane-nightly` at 04:15 UTC                         |
...
- `testing/five_node_suite.py` — Phases 1 and 2 are implemented and green against `docker-compose.5node.yml`: bootstrap, survives 2 voter failures, loses quorum at 3, recovers with acked writes intact, plus the partition shapes a 3-node cluster cannot express — a 3/2 split, a leader left with one follower, a three-way split where nobody writes, and `phase_rw1_asymmetric`, which runs the dual-writability prober across an asymmetric partition for RW-1. What is **not** implemented is membership chaos, 2-sync/2-async replication, and Elle at five nodes; the runner names those three on exit. Note the suite promotes learners to voters before asserting anything: compose starts all five joins at once and openraft 0.9 leaves the losers of that race as learners, which would make every quorum assertion vacuous.
...
A fault that silently fails to inject is worse than no test, because it reads as coverage. This has bitten seven times: `iptables` missing from the image; every fault `exec` running as unprivileged `postgres` (the image ends `USER postgres`, so `NET_ADMIN` on the container does not reach the process — privileged operations need `--user root`); three cases addressing docker objects by literal name while CI sets a per-run `COMPOSE_PROJECT_NAME`; a LazyFS fault worker parked before its read loop, so every `lazyfs::` command was accepted by the FIFO and never executed; and libfaketime moving `CLOCK_MONOTONIC` along with `CLOCK_REALTIME` on amd64, so the injected "wall-clock skew" moved the lease clock too and every clock-skew case on CI tested something other than what it claims (`FAKETIME_DONT_FAKE_MONOTONIC=1`, set in every compose file). Derive container and network names from the active compose project, never hardcode them, and assert the fault landed. `HARDENING.md` has the full list.

The last one is also the argument for running a case everywhere it is meant to run: it was invisible for as long as it existed because arm64's libfaketime does not fake the monotonic clock, so the oracle passed on a developer machine and had never once run in CI.
