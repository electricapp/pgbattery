...
Faults in the default matrix are all _clean_ (SIGKILL, container stop, network disconnect, SIGSTOP): `docker kill` leaves the host page cache intact, so none of them can discard an un-fsynced write. The dirty-crash path is separate — `testing/durability_crash.py` against `docker-compose.lazyfs.yml`, where PGDATA is a LazyFS mount and un-fsynced writes die with the process. That suite demonstrates W1 and R2 rather than asserting them, and refuses to report a green result until its inversion has gone red. Torn writes have a working injector (`arm_torn_write`, via `lazyfs::torn-op`) but no suite driving it in CI, and the Raft store sits outside the LazyFS mount so nothing can tear it yet (H-25). See `HARDENING.md`.
