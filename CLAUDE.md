...
| Join and rejoin edges  | `testing/join_edges.py`                      | A clone source deposed mid-copy, orphan slots (bounded against the reconciler's interval), a learner that dies mid-join, and a wiped bootstrap node returning on the cluster's data lineage rather than one it minted. |
