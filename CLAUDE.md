...
Faults in the default matrix are all _clean_ (SIGKILL, container stop, network disconnect, SIGSTOP): `docker kill` leaves the host page cache intact, so none of them can discard an un-fsynced write. The dirty-crash path is separate — `testing/durability_crash.py` against `docker-compose.lazyfs.yml`, where PGDATA is a LazyFS mount and un-fsynced writes die with the process. That suite demonstrates W1 and R2 rather than asserting them, and refuses to report a green result until its inversion has gone red. Torn writes are covered for PostgreSQL pages by `testing/durability_crash.py`'s sibling `testing/torn_write.py`, which tears a heap page via `lazyfs::torn-op` and asserts the page is repaired by the full-page image or rejected by checksums, never served silently; it too refuses a green until its inversion has gone red. The Raft store sits outside the LazyFS mount, so redb torn writes remain unproven (H-25). See `HARDENING.md`.
